...
playscreen=pygame.image.load('./images/goalScreen.png')
takoyaki3=pygame.image.load('./images/takoyaki3.png')
takoyaki4=pygame.image.load('./images/takoyaki4.png')
goalOverBg = pygame.image.load('./images/goalOverBg.png')
...
class GoalMode():
    ...
    def movableCook(self):   #function in which i am moving image
        ...
        if c_click[0] == 1 and c_x + imgWidth > c_mouse[0] > c_x and c_y + imgHeight > c_mouse[1] > c_y:  #asking if i am within the boundaries of the image
            if o_drag != 1:
                if b_drag != 1:
                    c_drag = 1                                                             #and if the left button is pressed
    ...
    def button_back(self, x, y, w, h, action=None):
        global end,final, b_x, b_y, o_x, o_y, c_x, c_y, money
        ...
        if 0 < cursor[0] < w and 0 < cursor[1] < h:
            if click[0] == 1 and action == "back":
                ...
                for a in taco_board.values():
                    a['status'] = None
                b_x, b_y, o_x, o_y, c_x, c_y, final, money = 540, 480, 680, 480, 830, 480, 0, 0
                while end:
                    end = False
    
    # 게임 끝
    def endGame(self):
        global final, b_x, b_y, o_x, o_y, c_x, c_y, money
        sucS.play()     #게임 끝 효과음
        gameDisplay.blit(font.render(str(money), True, (0, 0, 0)), (875, 360))
        gameDisplay.blit(goalOverBg, (0,0))
    ...
    # 메인 루프
    def main_loop(self):
        global final,money,b_x,b_y,o_x,o_y,c_x,c_y,cnt_1,cnt_2,cnt_3,end,cusChange
        self.rand()
        end = True
        while end:
            draw_y = 0
